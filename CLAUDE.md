...
5. If `KEYSTORE_BASE64`/`KEYSTORE_PASSWORD`/`KEY_ALIAS`/`KEY_PASSWORD` secrets are set, decodes the keystore and wires it in as the release `signingConfig` via `tool/patch_signing.js` (also builds `flutter build appbundle --release` alongside the APK — Play Console requires the `.aab` format). **These secrets are set** — CI produces a real signed `.aab`. The keystore itself (`dino-egg-shooter-release.jks`) lives gitignored in the project root; the user has been told to back it up externally, since losing it (or the password) means the app can never be updated under this identity again. See [[feedback_release_signing_setup]] in the memory system for the exact generation flow if a new keystore is ever needed for a sibling project.
